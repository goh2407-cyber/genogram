...
## 第五批：使用者追加（2026-09-03）

### L-1. 自動排列重寫 ✅
- **問題**：舊版走 dagre 泛用 DAG 佈局：配偶被拆到不同列或相距 1860px、只靠婚姻相連的人（前妻）被丟到最上列、子女離父母很遠，實際上沒人敢按。
- **做法**：`js/layout.js` v3 自家分層佈局（Sugiyama-lite）：配偶 union-find 同群、同一父母的子女同群 → 群 DAG 最長路徑定輩分，再「壓實」把只有子女約束的父母貼到子女列上一列；同列以「單位」（婚姻相連的一串人）排：本人在中間、配偶依婚期由近到遠向外；子女塊以「首尾手足中點」對齊父母中點；手足年齡長→幼，年齡未知者依重心（本輪期望位置）決定，down/up 多輪迭代；同列排序以「手足叢集」為單位，不會被別家單位插隊。列距 = CELL_HEIGHT、X 吸附半格。獨立人物（沒有任何關係）原地不動也不吸附。生活圈跟隨、預覽／套用／取消流程沿用。
- **驗證**：`verify_auto_layout.js` 15/15（三組資料：案主三代＋前妻＋無關人物、使用者版面三前妻、26 人範例）；預覽流程既有測試綠。
- **順帶**：三份 index.html 移除 dagre script（不再需要，離線包少一個檔）。

### R-1. 婚姻線橫桿距離可調 ✅
- **問題**：使用者：「右邊多段婚姻或關係線高度有沒有辦法調整，想擺好看但線會卡到角色」— ㄇ 天橋固定頭頂 20px＋每層 30px、ㄩ 固定文字下緣＋14px，多段婚姻時天橋常壓到上方人物。
- **做法**：`Relationship.routeLift`（px ≥ 0，只在有值時寫入 JSON，舊檔不變）；`getMarriageConfiguration` 對 over 減、under 加 lift，幾何快取簽章含 routeLift；auto／一 不受影響。面板「婚姻線走法」下新增「橫桿距離 －／值／＋／重設」（auto／一 時停用並提示）；畫布上選取婚姻線後可直接按住橫桿上下拖（`getMarriageBarAt`，游標 ns-resize），放開才寫一筆 history。日期文字與子女掛接點沿用同一幾何自動跟隨；匯出與螢幕共用。
- **驗證**：`verify_route_lift.js` 14/14；golden 16/16（lift=0 幾何不變）。

## 明確不做（沿用 roadmap §4 + 使用者決定）
- ~~自動佈局 / couple-vertex packer（2026-06-18 決定）~~ → 2026-09-03 使用者要求下已重寫「自動排列」（L-1）；仍是手動優先、排列只是可取消的預覽。
...
| 2026-09-03 | L-1 自動排列重寫（Sugiyama-lite） | verify_auto_layout 15/15 | 不再依賴 dagre；獨立人物不動 |
| 2026-09-03 | R-1 婚姻線橫桿距離可調 | verify_route_lift 14/14；golden 16/16 | 面板 ＋／－／重設 + 畫布拖橫桿 |
