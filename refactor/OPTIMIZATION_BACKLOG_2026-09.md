...
| 第三批：需要設計拍板 | 4 | 0 | 去識別化、觸控、LOD、拆檔（2026-09-03 使用者同意全做） |
| 第四批：同住框與生活圈 | 7 | 0 | HH-1～4、LC-1～3 |
...
### HH-1. 同住框邊界納入姓名／備註文字 ✅→做
- [ ] `getHouseholdBounds` 取樣點加入成員 `getPersonLabelGeometry` 的文字區塊四角（含 padding），框線不再切到姓名
- [ ] draw / hit-test / 匯出邊界共用同一份 bounds（原本就共用，不另分支）
- 驗證：`verify_hh_lc.js` H1–H7 綠；新增檢查「成員姓名區塊完全在框內」
- 狀態：

### HH-2. 同住框名稱顯示於框上 + 圖例
- [ ] `household.label`（可空）顯示在框頂部（白 halo），螢幕與匯出一致；面板加「名稱」欄（備註維持面板內）
- [ ] 匯出圖例「特殊」段加一筆「虛線框 = 同住」；側欄圖例同步
- 驗證：`verify_legend_consistency` 綠；roundtrip 保留 label
- 狀態：

### HH-3. 成員增減不必重圈
- [ ] 面板成員改成可移除的標籤（✕）；「把目前選取的人加入此框」按鈕；成員歸零自動刪框
- 驗證：新檢查（移除／加入／undo）
- 狀態：

### HH-4. 已選好人直接按 H 建框
- [ ] 已有多選（Shift 或框選）時按 H／點同住工具 → 直接建立，不必再點人與 Enter
- 狀態：

### LC-1. 生活圈頂點可編輯
- [ ] 選取狀態下拖單一頂點；雙擊邊線插入頂點；Alt+點頂點刪除（至少保留 3 點）；皆進 history
- 驗證：新測試 `verify_lifecircle_edit.js`
- 狀態：

### LC-2. 生活圈畫法更快
- [ ] 生活圈工具下「拖一下」拉橢圓（放開轉 16 點多邊形）；點頂點畫法保留；畫的過程 Backspace 退回上一個頂點
- 狀態：

### LC-3. 生活圈標籤與說明
- [ ] 標籤位置可選（頂部／中央／底部）；面板加「說明」欄（不畫在畫布，JSON 保留）
- 狀態：
